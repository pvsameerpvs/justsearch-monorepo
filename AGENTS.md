...
## 14. Database Architecture (Supabase + Drizzle ORM)

### 14.1 Overview

| Component | Technology | Purpose |
|-----------|-----------|---------|
| **Database** | Supabase PostgreSQL | Managed multi-tenant database |
| **ORM** | Drizzle ORM | Type-safe queries, schema definitions |
| **Migration** | Drizzle Kit | CLI for schema migrations |
| **Driver** | `postgres` (npm) | Fast PostgreSQL driver |
| **Auth** | Supabase Auth | JWT, email/phone authentication |
| **Backup** | Supabase auto + custom script | Daily backups + per-tenant export |
| **Tenant Strategy** | Schema-per-Restaurant | Strong data isolation |

### 14.2 Why Schema-Per-Restaurant

Each restaurant gets its own PostgreSQL schema (namespace). Same table names exist in every schema with different data.
...
```sql
public.restaurants
  - id (uuid), slug (varchar), schema_name (varchar), name (varchar)
...
```
User visits: mosaic.justsearch.com
    |
Extract subdomain: "mosaic"
    |
Query: SELECT schema_name FROM public.restaurants WHERE subdomain = 'mosaic'
    |
Result: schema_name = "rest_mosaic"
    |
All queries: SELECT * FROM rest_mosaic.orders
    |
Restaurant A sees only Restaurant A data
```

### 14.6 Backend File Structure
...
│   │   ├── schema/
│   │   │   ├── public.schema.ts      # public schema tables
│   │   │   └── tenant.schema.ts      # per-tenant tables
...
│   │   └── games/
│   ├── middleware/
│   │   ├── tenant.middleware.ts      # Subdomain to schema resolver
│   │   └── auth.middleware.ts
│   └── types/
│       └── db.types.ts
├── drizzle.config.ts
...
|---------|-----------|----------|
| Daily automated backups | 7 days | 30 days |
| Point-in-time recovery | No | Yes |

**Custom Per-Tenant Backup Script:**
...
|-------|------|-------------|
| 1. Setup | 1 | Drizzle ORM installed, connected to Supabase |
| 2. Public Schema | 1 | `restaurants`, `games`, `advertisements` tables |
| 3. Tenant System | 2 | Schema creation, subdomain resolver middleware |
| 4. Auth | 2 | Users table, Supabase Auth, roles |
| 5. Core Tables | 3 | Orders, menus, menu_items, drivers, staff |
| 6. API Routes | 3 | REST API for all CRUD |
