...
export function DetailsStep({ form, mode, autoSwitched, busy, error, onRequestOtp, canRequestOtp }: DetailsStepProps) {
    ...
    <div className="grid gap-3">
      ...
      <label className="grid gap-1.5">
        ...
        <Controller
          ...
          render={({ field }) => (
            <div className={`flex h-12 items-center gap-2 rounded-2xl border px-4 text-sm font-medium outline-none transition-all ${autoSwitched ? 'border-[rgb(var(--border)/0.5)] bg-[rgb(var(--card-surface-muted)/0.4)] text-[rgb(var(--muted))]' : 'border-[rgb(var(--border)/0.9)] bg-white text-[rgb(var(--ink))] focus-within:border-[rgb(var(--brand))] focus-within:ring-4 focus-within:ring-[rgb(var(--brand)/0.12)]'}`}>
              <span className="select-none font-mono text-[rgb(var(--muted))]">+971</span>
              <input
                value={field.value}
                onChange={(event) => field.onChange(normalizeUaeLocalDigits(event.target.value))}
                placeholder="501234567"
                className="h-full w-full bg-transparent font-mono outline-none"
                inputMode="numeric"
                autoComplete="tel"
                disabled={autoSwitched}
                readOnly={autoSwitched}
              />
              {autoSwitched && (
                <span className="ml-auto shrink-0 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-600">
                  Verified
                </span>
              )}
            </div>
        ...
        />
        {autoSwitched ? (
          <span className="text-xs text-[rgb(var(--muted))]">Number confirmed. Enter your name above to continue.</span>
        ) : (
      ...
      {autoSwitched && (
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 px-4 py-3 text-sm font-medium text-emerald-700">
          New number detected. Please enter your name to complete sign up.
        </div>
      )}
      ...
      <button type="button" disabled={!canRequestOtp || busy} onClick={onRequestOtp} className="inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[rgb(var(--brand))] px-5 text-sm font-semibold text-white shadow-[0_14px_36px_rgb(var(--brand)/0.25)] transition-all hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60 active:scale-[0.99]">
        {busy ? 'Sending OTP…' : (mode === 'login' ? 'Send OTP to Login' : 'Send OTP to Sign Up')}
      </button>
    </div>
  );
}
