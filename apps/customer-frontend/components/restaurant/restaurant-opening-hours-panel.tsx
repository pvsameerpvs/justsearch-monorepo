...
export function RestaurantOpeningHoursPanel({
  ...
  return (
    <Surface className="rounded-[32px] border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgb(var(--accent-soft)/0.3))] p-6 sm:p-7">
        ...
        <div className="w-full">
          ...
          <div className="mt-5 space-y-3">
            {openingHours.map((openingHour) => (
              <div
                key={openingHour.day}
                className="flex items-center justify-between rounded-[20px] border border-[rgb(var(--card-border)/0.86)] bg-[rgb(var(--card-surface)/0.82)] px-4 py-3 text-sm text-[rgb(var(--muted))]"
              >
