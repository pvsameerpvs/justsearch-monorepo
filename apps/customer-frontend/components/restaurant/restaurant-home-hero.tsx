...
import { mapApiOrdersToSummaries } from './checkout/use-api-order-summaries';
import { getCheckoutStatusHref } from './checkout/checkout-live-status-utils';
        ...
        <Surface className="relative mx-auto max-w-5xl rounded-[32px] border-[rgba(var(--card-border),0.9)] bg-[linear-gradient(145deg,rgba(var(--brand-soft),0.45),rgba(var(--card-surface),0.96),rgba(var(--accent-soft),0.48))] p-6 sm:p-8 lg:p-5">
          <div className="relative z-10 flex flex-col items-center gap-5 text-center sm:gap-6 lg:gap-4">
            ...
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[rgb(var(--brand))]">{restaurant.category}</p>
              <div className="flex items-center justify-center gap-3 flex-wrap">
                <h2 className="font-display text-4xl font-semibold tracking-[-0.08em] text-[rgb(var(--ink))] sm:text-5xl lg:text-4xl xl:text-5xl">{restaurant.name}</h2>
              ...
              <p className="mx-auto max-w-2xl text-sm leading-6 text-slate-600 sm:text-base lg:text-sm lg:leading-5">{restaurant.tagline}</p>
              {restaurant.description && (
                <p className="mx-auto max-w-2xl text-xs leading-5 text-slate-500 sm:text-sm lg:text-xs lg:leading-4">{restaurant.description}</p>
              )}
              {restaurant.cuisine.length > 0 && (
                <div className="flex flex-wrap justify-center gap-2">
                  {restaurant.cuisine.map((c, i) => (
                    <span key={`${c}-${i}`} className="rounded-full border border-[rgb(var(--card-border))] bg-[rgb(var(--card-surface))] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[rgb(var(--muted))]">{c}</span>
                  ))}
                </div>
              )}
