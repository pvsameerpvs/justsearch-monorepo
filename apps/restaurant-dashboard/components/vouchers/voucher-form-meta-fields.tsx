...
export function VoucherFormMetaFields({ control }: Props) {
  return (
    <div className="space-y-3">
      <Controller name="usageLimit" control={control}
        render={({ field, fieldState }) => (
          <FormField label="Usage Limit" icon={Hash} error={fieldState.error?.message}>
            <input type="number" min={1} {...field} onChange={(e) => field.onChange(Number(e.target.value))}
              placeholder="100" className={`elegant-input w-full ${fieldState.error ? "border-red-300" : ""}`} />
          </FormField>
        )} />
        ...
        <Controller name="startDate" control={control}
          render={({ field, fieldState }) => (
            <FormField label="Start Date" icon={CalendarDays} error={fieldState.error?.message}>
              <input type="date" {...field} className={`elegant-input w-full ${fieldState.error ? "border-red-300" : ""}`} />
            </FormField>
          )} />
