...
import { VoucherAssignBanner } from "./voucher-assign-banner";
...
import { ScratchCardSettings } from "./scratch-card-settings";

export function VoucherManager() {
  const { vouchers, stats, isLoading, error, refetch, filterStatus, setFilterStatus,
    showForm, setShowForm, editingId, setEditingId, deletingId, setDeletingId,
  ...
  if (isLoading) return (
    <div className="space-y-5">
      ...
      <ScratchCardSettings /><VoucherSkeleton />
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      {justCreatedCode && <VoucherAssignBanner code={justCreatedCode} onDismiss={() => setJustCreatedCode(null)} />}
      {vouchers.length === 0 ? <VoucherEmpty onAdd={() => setShowForm(true)} /> : <VoucherList vouchers={vouchers} onEdit={setEditingId} onDelete={setDeletingId} onToggle={toggleActive} />}
      {(showForm || editingId) && <VoucherFormModal voucher={editingVoucher} onSave={onSave} onCancel={() => { setShowForm(false); setEditingId(null); }} />}
      {deletingVoucher && <VoucherDeleteModal voucher={deletingVoucher} onConfirm={onDelete} onCancel={() => setDeletingId(null)} />}
    </div>
  );
}
