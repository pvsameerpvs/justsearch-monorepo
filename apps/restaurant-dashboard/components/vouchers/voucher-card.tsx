...
export function VoucherCard({ voucher, onEdit, onDelete, onToggle }: VoucherCardProps) {
  const isPercentage = voucher.type === "percentage";
  const discountLabel = isPercentage ? `${voucher.value}% OFF` : `AED ${voucher.value} OFF`;
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="text-right">
            <p className="text-lg font-bold text-amber-600">{discountLabel}</p>
            <p className="text-[10px] text-slate-400">
              {isPercentage ? `Max AED ${voucher.maxDiscount}` : "Fixed amount"}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 text-[10px] text-slate-400 mb-3">
          <div className="flex items-center gap-1">
            ...
            <span>Min AED {voucher.minOrderValue}</span>
          </div>
          <div className="flex items-center gap-1">
